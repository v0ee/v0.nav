...
class MultiBotManager extends EventEmitter {
    ...
    startInstance(instance) {
        if (!instance || !instance.id) {
            const errorDetail = instance ? `id is ${instance.id}` : 'instance is null/undefined';
            this.forwardSystemLog?.(`Cannot start instance: invalid instance data (${errorDetail})`, 'red');
            this.logger?.logError?.('startInstance failed', new Error('Invalid instance data'), { instance });
            return null;
        }
    ...
    stopInstance(instanceId, reason = 'manual stop') {
        ...
        if (!entry) {
            this.forwardSystemLog?.(`Cannot stop instance: "${instanceId}" is not running`, 'yellow');
            this.logger?.logWarning?.('stopInstance called on non-running instance', { instanceId, reason });
            return false;
        }

        try {
            this.forwardSystemLog?.(`Stopping instance: ${entry.instance.name} (reason: ${reason})...`, 'yellow');
            ...
            this.forwardSystemLog?.(`Error stopping instance "${entry.instance.name}": ${err.message}`, 'red');
            this.logger?.logError?.('stopInstance error', err, { instanceId, instanceName: entry.instance.name, reason });
            return false;
        }
    }
